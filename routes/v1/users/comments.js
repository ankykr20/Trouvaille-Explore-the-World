...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const comments = await Query(
      "SELECT c.com_id, c.parent_id, c.created_at, c.comment, c.user_id, u.user_name, u.dp FROM comments c 
        INNER JOIN user_details u ON u.user_id = c.user_id WHERE c.post_id = ? ORDER BY c.created_at ASC",
      [post_id]
    );
