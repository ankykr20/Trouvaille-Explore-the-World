...
router.post("/home", auth, async (req, res) => {
  try {
    ...
    res.send(
      JSON.stringify({ followersRes, radiusId, followingId, postsId, isEnd })
    );
  } catch (err) {
    console.log(err);
    res.status(400).send(err);
...
router.post("/all", auth, async (req, res) => {
  try {
    ...
    let posts = await Query(
      `SELECT DISTINCT p.*, u.user_name, u.dp FROM posts p INNER JOIN user_details u ON p.user_id = u.user_id 
      INNER JOIN tags t ON t.post_id = p.post_id WHERE p.status = '1' AND t.tag_id IN ${tagIds} AND p.user_id != ? 
      ORDER BY time DESC`,
      [_id]
    );
